...
function FollowerList() {
    ...
    const [pageTitle, setPageTitle] = useState("フォロワー");
            ...
            try {
                ...
                if (targetUserId) {
                    ...
                    if (userData.data?.username) {
                        setPageTitle(`${userData.data.username}のフォロワー`);
                    }
                } else {
                ...
                // フォロワーのレコードをGSI（followeeId）で取得
                const { data: follows } = await client.models.Follow.listFollowByFolloweeId({
                    followeeId: userId,
                });
                ...
                const userResults = await Promise.all(
                    follows.map((f) => client.models.User.get({ userId: f.followerId }))
                );
